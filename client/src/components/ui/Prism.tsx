...
export const Prism: React.FC<PrismProps> = ({
  ...
  useEffect(() => {
    ...
    const NUM_STEPS = Math.max(16, Math.min(100, steps || 38));

    // Cap DPR at 1.0 (or custom) for massive GPU fill-rate optimization
    const dpr = customDpr || Math.min(1.0, typeof window !== 'undefined' ? window.devicePixelRatio || 1 : 1);
    ...
    const fragment = /* glsl */ `
      ...
      void main(){
        ...
        for (int i = 0; i < STEPS; i++) {
          ...
          vec3 q = p;
          q.y += centerShift;
          d = 0.12 + 0.22 * abs(sdPyramidUpInv(q));
          z -= d;
          o += (sin((p.y + z) * cf + vec4(0.0, 1.0, 2.0, 3.0)) + 1.0) / d;
        }

        // Compensate for step count reduction
        float normFactor = float(STEPS) / 100.0;
        o = tanh4((o / normFactor) * (o / normFactor) * (uGlow * uBloom) / 1e5);
    ...
    const targetInterval = 1000 / Math.max(15, Math.min(60, fpsLimit || 40));
    let lastRenderTime = 0;

    const render = (t: number) => {
      // Throttle render loop to target FPS (saves GPU cycles on 144Hz/240Hz monitors)
      const elapsed = t - lastRenderTime;
      if (elapsed < targetInterval) {
        raf = requestAnimationFrame(render);
        return;
      }
      lastRenderTime = t - (elapsed % targetInterval);
